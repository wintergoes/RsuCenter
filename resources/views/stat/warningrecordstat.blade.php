        ...
        <form id="form1" class="form-horizontal" method="get" >
            ...
            <table style="font-size: 12px; text-align: center;" >
                <tr>                   
                    ...
                    <td class="search_td">&nbsp;&nbsp;Obu设备：&nbsp;&nbsp;</td>
                    <td class="search_td">
                        <select name="obuid" id="obuid" class="form-select" >
                            <option value="-1" >不限</option>
                            @foreach ($obus as $obu)
                            <option value="{{$obu->id}}" >{{$obu->obuid}}</option>
                            @endforeach
                        </select>                        
                    </td>
			...
			<div class="card-body">
                            <ul class="nav nav-tabs nav-primary mb-3" role="tablist">
                                    <li class="nav-item" role="presentation">
                                            <a class="nav-link active" data-bs-toggle="tab" href="#warningrecordstatChart" role="tab" aria-selected="true">
                                                    <div class="d-flex align-items-center">
                                                            <div class="tab-icon"><i class="bx bx-chart font-18 me-1"></i>
                                                            </div>
                                                            <div class="tab-title">预警次数趋势图</div>
                                                    </div>
                                            </a>
                                    </li>
                                    <li class="nav-item" role="presentation">
                                            <a class="nav-link" data-bs-toggle="tab" href="#warningrecordstatTable" role="tab" aria-selected="false">
                                                    <div class="d-flex align-items-center">
                                                            <div class="tab-icon"><i class="bx bx-table font-18 me-1"></i>
                                                            </div>
                                                            <div class="tab-title">预警次数列表</div>
                                                    </div>
                                            </a>
                                    </li>
                            </ul>                            
                            ...
                            <div class="tab-content py-2">
                                <div class="tab-pane fade active show" id="warningrecordstatChart" role="tabpanel">
                                ...
                                <div class="tab-pane fade" id="warningrecordstatTable" role="tabpanel">
...
function drawChart(){
    ...
    //alert($("#obuid").find("option:selected").val());
    
    $.ajax({
        type: "POST",
        url: "warningrecordcountstatjson?fromdate=" + $("#fromdate").val() + "&todate=" + $("#todate").val() + "&obuid=" +  $("#obuid").find("option:selected").val(),
        dataType: "json",
        success: function (data) {
            ...
            var celldate = tr1.insertCell(0);
            celldate.innerHTML = "日期";
            var celltitle = tr1.insertCell(1);
            celltitle.innerHTML = "预警次数";
            
            var dataary = [];
            var tecdata = {};
            for(var i = 0; i < data.wrcdata.length; i++){
                var trdata=ta.insertRow(ta.rows.length);
                var celldate = trdata.insertCell(0);
                celldate.innerHTML = data.wrcdata[i].ddate;
                var cellcount = trdata.insertCell(1);
                cellcount.innerHTML = data.wrcdata[i].wrcount;                        

                dataary.push(data.wrcdata[i].wrcount);
            }
...
function drawEventTypeStat(){
    ...
    $.ajax({
        type: "POST",
        url: "warningrecordeventtypestatjson?fromdate=" + $("#fromdate").val() + "&todate=" + $("#todate").val() + "&obuid=" +  $("#obuid").find("option:selected").val(),
        dataType: "json",
        success: function (data) {
            var etdataset = [];
            var etlabels = [];
...
function drawEventSourceStat(){
    ...
    $.ajax({
        type: "POST",
        url: "warningrecordeventsourcestatjson?fromdate=" + $("#fromdate").val() + "&todate=" + $("#todate").val() + "&obuid=" +  $("#obuid").find("option:selected").val(),
        dataType: "json",
        success: function (data) {
            var etdataset = [];
            var etlabels = [];
